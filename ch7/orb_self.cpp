...
int main(int argc, char **argv) {
    ...
    //load image
    Mat image_1 = imread(first_file,0);  //灰度图读入
    Mat image_2 = imread(second_file,0); //灰度图读入

    //FAST point detect
    chrono::steady_clock::time_point t1 = chrono::steady_clock::now();
    vector<KeyPoint> keypoint_1;
    FAST(image_1,keypoint_1,40);
    vector<DescType> descriptor_1;

    vector<KeyPoint> keypoint_2;
    FAST(image_2,keypoint_2,40);
    vector<DescType> descriptor_2;

    ComputeORB(image_1,keypoint_1,descriptor_1);
    ComputeORB(image_2,keypoint_2,descriptor_2);
    ...
    //image show
    Mat image_show;
    drawMatches(image_1,keypoint_1,image_2,keypoint_2,matches,image_show);
...
void BfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for(size_t i=0;i<desc1.size();i++){
        ...
        for(size_t j=0;j<desc2.size();j++){
            ...
            int dis = 0;
            for(int k=0;k<8;k++){
                dis += _mm_popcnt_u32(desc1[i][k] ^ desc2[j][k]);
            }
            if(dis < d_max && dis < m.distance){
                m.distance = dis;
                m.trainIdx = j;
            }
            cout<<"dis = "<<dis<<endl;
